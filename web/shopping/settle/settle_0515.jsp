			   ...
			   <div id="content_list">
				  <div class="showlist" id="two">
				     <table class="table_style" border="0" id="table_test">
				             ...
				             <th class="table_th" style="font-weight:normal;text-align: left;">支付项目<select  name="pro" class="select_pros" id="pro">
				                     <option value="0">==请选择支付项目==</option></select>
										<span>项目金额<input type="text" style="width:64px;"></span>
				                        <span id="proinfo" class="proinfos" style="display: inline;">项目负责人：黄先开；项目余额：163341.3</span>
				                        <span style="display:none;" id="proid" class="proids">81131431403</span><!--项目-->
				                        <span style="display:none;" id="chargecode" class="chargecodes"></span><!--项目负责人编号-->           
				                        <span style="display:none;" id="chargename" class="chargenames">黄先开</span><!--项目负责人名字-->                            
				                    <!-- <span id="add">+</span> -->
...
$(function(){
	...
	$("#add").click(function(){
		...
		$("#table_test").append('<tr><td><span class="view_details">支付项目<select name="pro" class="select_pros">'+select_html+'</select></span><span>项目金额<input type="text" style="width:64px;"></span> <span id="proinfo" class="proinfos" style="display: inline;">项目负责人：黄先开；项目余额：163341.3</span> <span style="display:none;" id="proid" class="proids">81131431403</span><span style="display:none;" id="chargecode" class="chargecodes"></span><span style="display:none;" id="chargename" class="chargenames">黄先开</span><span class="add_myclass" id="display"  onclick="deleteTr(this)"><img src="./images/close.png"/style="vertical-align:middle; margin-bottom:5px;cursor: pointer;width:22px; height:22px;"></span>'+'</td></tr>');
		i++;
		
	});
	
	$("#table_test .select_pros").change(function(){
		
		var value = $(this).val();
		var strs= new Array();
		//alert(value);
		if(value == 0){
			$(this).next(".proinfos").hide();
		}else{
			strs=value.split("|,|");
			var h3 = $(this).parents("tr").html();
			var h3 = $(this).parents("tr").find(".proinfos").html();
			$(this).parents("tr").find(".proinfos").html("项目负责人："+strs[2]+"；项目余额："+strs[3]);//项目编号："+strs[1]+"；项目名称："+strs[0]+");
			$(this).parents("tr").find(".paytypes").html("项目支付");
			$(this).parents("tr").find(".chargenames").html(strs[2]);
			$(this).parents("tr").find(".proids").html(strs[1]);
				
			$(this).parents("tr").find(".proinfos").show();
		}
