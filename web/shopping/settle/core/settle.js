...
window.onload=function(){
	...
	$("#add").click(function(){
		...
		$("#list1").append('<td>支付项目<select  name="pro" >'+select_html+'</select><span> 项目金额<input type="text" style="width:64px;" name = "pro_money"/></span> <span  style="display: inline;" class="proinfo"></span> <span     class="proid" style="display:none;"></span><span style="display:none;"  class="chargecode"></span><span     class="chargename" style="display:none;"></span><span class="add_myclass"    onclick="deleteTr(this)"><img src="./images/close.png"/style="vertical-align:middle; margin-bottom:5px;cursor: pointer;width:22px; height:22px;"/></span>'+'</td>');
		i++;
	...
	/**选择付费项目**/
	$('.table_style select[name="pro"]').live("change",function(){
		
		var strs= new Array(); 
		var data = $(this).val();
		if(data.value==0){
			$(this).siblings(".proinfo").hide();

		}else{
			strs=data.split("|,|");
			//项目编号："+strs[1]+"；项目名称："+strs[0]+"；
			$(this).parents(".addp").find(".chargename").html(strs[2]);
			$(this).parents(".addp").find(".proid").html(strs[1]);
			$(this).parents(".addp").find(".proinfo").html("项目负责人："+strs[2]+"；项目余额："+strs[3]);	
			$(this).parents("tr").find(".proinfo").show();
		}
	});
...
function submitorder(cartids){
	...
	/**05-15 白雪 多个支付项目**/
	//多个项目代码
	var pro_codes = $("[name='pro']:checked");
	var pro_json = "[";
	var proj = $("[name='pro']");
	var prol = $("[name='pro']").length;
	for(var i = 0;i < prol; i++) { 
		project_code = proj.eq(i).val();
		if (project_code==0){
			alert('请选择支付方式');
			return;
		}else{
			var substr = project_code.split("|");
		}
		pro_json+="{procode:'"+substr[2]+"'},";
	}
	pro_json=pro_json.substring(0,pro_json.length-1);
	pro_json +=']';
	//每个付费项目的资金
	var pro_moneys = $("[name='pro_money']");
	var prom_json = "[";
	var promj = $("[name='pro_money']");
	var proml = $("[name='pro_money']").length;
	
	for(var i = 0;i < proml; i++) { 
		project_money = promj.eq(i).val();
	
		if (project_money==""){
			alert('请填写项目金额');
		return;
		}
		prom_json+="{pro_money:'"+project_money+"'},";
	}
	prom_json=prom_json.substring(0,prom_json.length-1);
	prom_json +=']';
	if (prol != proml){
		alert('请填写项目金额');
		return;
